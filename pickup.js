...
// Big boy functions
  ...
  start_game(max_teams = this.max_teams, team_size = this.team_size) {
    // Starts a game, duh!
    // Can create pickups with varying number of teams
    // and team sizes.
    this.game_on = true;
  ...
  random_add(nick) {
    ...
    // Returns true or false.
    // Maybe try rewrite this sometime?
    let arr = []
    ...
    if (team > -1) {
      ...
      console.log("All teams are full?"); // Redundant, I know.
      return false;
    }
  }

  remove(nick) {
    // Removes a player from any team.
    // Returns true or false.
    if (this._is_added(nick)) {
      for (let i = 0; i < this.teams.length; i++) {
